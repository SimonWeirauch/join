...
async function userSelection(isClosed) {
  ...
  if (isOpen) {closeUserSelection(selectedUser, select, selectBG)}
  else {renderUsersInUserselection(select, search, isClosed);}
  if (!isClosed) {whoAmi();}
}
...
/**
 * renders the user in the user selection 
 * @param {Element} select element user_selection
 * @param {Element} search element search_contact
 * @param {Integer} i index
 */
function renderUser(select, search, i){
