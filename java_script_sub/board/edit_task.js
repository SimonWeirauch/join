...
async function getTaskValues(currentTask) {
  ...
  setTaskValues(currentTaskData, actions);
}


/**
 * Sets Input-Values of the given Task (like Title, Description)
 * @param {Array-Element} currentTaskData the current task in the task array
 * @param {Array} actions an array with all fields of a single task
 */
function setTaskValues(currentTaskData, actions) {
