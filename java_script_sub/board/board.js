...
function updateHTML(search) {
  let longText = ['No tasks To do','No tasks in progress',
                  'No await feedback','No tasks done',];
...
/**
 * prevent default on dragged element
...
/**
 * displays the member who are assigned to the specific task
 * @param {} task 
 * @returns HTML-Element with the rest amount of members assinged to the task
 */
  ...
  for (let i = 0; i < task['member'].length; i++) {
    ...
    for (let j = 0; j < contactsTask.length; j++) {
      ...
      if (contactTask['register_entry'][0]['contact_ID'] === element) {
        if (counterMember > 3) {rest++;} 
        else {
          addProfileBadgeToTask(contactTask, pixelLeft, counterMember);
          pixelLeft = pixelLeft + 8;
        }
        counterMember++;}}}
  if (rest > 0) {return (restHTML += /*html*/ `<div class="rest">+ ${rest}</div>`);}
}


/**
 * adds the profile badges to a task that the person is assigned to
 * @param {Object} contactTask the current task object
 * @param {integer} pixelLeft the amount of pixels the badges has to be move to the left to be overlapped
 * @param {integer} counterMember the amount of member assigned to the task 
 */
function addProfileBadgeToTask(contactTask, pixelLeft, counterMember){
